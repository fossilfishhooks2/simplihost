...
#define MAX_PATH_SIZE 16384 // Choose an appropriate size for paths
#define MAX_HEADER_SIZE 16384 // Choose an appropriate size for headers
...
int main(int argc, char* argv[]) {
    if (argc == 2) {
        if (strcmp(argv[1], "help") == 0) {
            printf("Simplihost 4.1.0 starting...\n");
            printf("Simplihost is a simple and lightweight web server. Like all websites, it hosts on port 80. The default pessimistic transfer time is 2000 ms. You can change this at runtime by passing the new value into the first argument (ex. simplihost.exe 1000).\n");
            return 0;
        }
        else {
            transfer = atoi(argv[1]);
        }
    }
    printf("Simplihost 4.1.0 starting...\n");
    Sleep(1000);
